...
import { Upload, X, Book as BookIcon, CheckCircle2, ArrowLeft } from 'lucide-react';
...
export default function AdminBookCreatePage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                setSuccess(true);
                setTimeout(() => router.push('/admin/books'), 2000); // Redirect to admin book list
            } else {
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <button onClick={() => router.back()} className="flex items-center gap-2 text-muted-foreground hover:text-foreground mb-2 transition-colors text-sm">
                        <ArrowLeft size={16} /> Back
                    </button>
                    <h1 className="text-2xl font-bold text-foreground tracking-tight">Upload New Book</h1>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3 bg-card/50 border border-border p-4 rounded-xl shadow-sm hover:shadow-md dark:shadow-none transition-shadow duration-300">
                                <input
                                    type="checkbox"
                                    id="is_premium"
                                    className="h-4 w-4 rounded border-zinc-700 bg-muted text-indigo-600 focus:ring-indigo-500"
                                    checked={formData.is_premium}
                                    onChange={e => setFormData({ ...formData, is_premium: e.target.checked })}
                                />
                                <label htmlFor="is_premium" className="text-sm font-medium text-foreground">Premium Content</label>
                            </div>
                        </div>
